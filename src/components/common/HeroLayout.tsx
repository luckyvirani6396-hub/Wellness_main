...
export default function HeroLayout({
  ...
  bottomBar,
  extraContent,
  sectionClassName = 'pt-10 pb-24 lg:pt-0 lg:pb-0',
}: HeroLayoutProps) {
  return (
    <section className={`relative overflow-hidden bg-gradient-to-br from-[#FFFDF8] via-[#F8F7F2] to-[#EEF5E8] min-h-screen lg:flex lg:items-center ${sectionClassName}`}>
          ...
          <div className="col-span-1 lg:col-span-6 flex flex-col items-start text-left relative z-10">
              ...
              <nav className="flex items-center gap-1.5 text-xs font-extrabold tracking-wider uppercase text-[#004B57]/90 flex-wrap">
                {breadcrumbs.map((crumb, i) => (
                  <span key={crumb.label} className="contents">
                    {i > 0 && <span className="text-[#004B57]/30">/</span>}
                    {crumb.to ? (
                      <Link to={crumb.to} className="hover:text-[#5c8730] transition-colors duration-200">{crumb.label}</Link>
                    ) : (
                      <span className="text-[#5C8730]">{crumb.label}</span>
                    )}
                  </span>
                ))}
              </nav>
            ...
            {highlights && (
              <motion.div variants={fadeUp} initial="hidden" animate="visible" className="relative flex items-start justify-between w-full max-w-[340px] sm:max-w-[420px] mb-10 px-2">
                ...
                {highlights.map(({ key, icon: Icon, label }) => (
